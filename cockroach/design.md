...
大多时候，时间戳缓存的更新相当于Key正在被读，尽管时间戳缓存也保护一些写操作（这些写操作最终一定位于cache内）
...
分布式SQL作为2016Q3新的执行框架，其目标是分布式SQL查询处理。
...
- 我们希望在持有数据的节点上执行查询和更新操作，如`UPDATE .. WHER` 和 `DELETE .. WHERE` 这样的语句(相对来说通过网络的网关接收结果，然后执行更新或删除，涉及额外的往返)。
...
- _逻辑执行计划_ - 表面上类似于在[SQL](#sql)章节中`planNode` 树描述,在数据计算阶段表达抽象（非分布式）数据流
- _物理执行计划_ - 一个物理执行计划是逻辑执行计划节点到CockroachDB节点上概念的映射。逻辑执行计划节点被复制和被特指依赖于集群拓扑。
  物理执行计划的组件在集群上调度和运行。
...
特殊的 **表读者** 聚合没有数据流，被用作数据源； 作为需求一个表读者可以被配置到指定的列且只有输出流。
特殊的 **final** 聚合没有输出流，被用作查询/声明的结果。
